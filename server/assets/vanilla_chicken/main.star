# vanilla_chicken — the THIRD 1:1 vanilla-mob dogfood (MOB-PASS-03). The vanilla Chicken's
# PASSIVE-AMBIENT AI re-expressed AS a Starlark plugin, staying a LITERAL method-for-method port of
# the unobfuscated 26.2 jar (temp/cache/26.2-inner.jar, javap -c -p). It declares the SAME 8 passive
# goals net.minecraft.world.entity.animal.chicken.Chicken.registerGoals installs, reusing the proven
# Go goal runtime (the same starlarkGoal seams the vanilla_pig dogfood drives), at the EXACT
# priorities + flags + params read from the bytecode (34-JARNOTES.md:38-48).
#
# Chicken.registerGoals() (jar-verified, 34-JARNOTES.md:38-48):
#   @0 FloatGoal, @1 PanicGoal(1.4), @2 BreedGoal(1.0), @3 TemptGoal(1.0, is(CHICKEN_FOOD), false),
#   @4 FollowParentGoal(1.1), @5 WaterAvoidingRandomStrollGoal(1.0), @6 LookAtPlayerGoal(Player, 6.0),
#   @7 RandomLookAroundGoal.
# isFood = is(ItemTags.CHICKEN_FOOD). The chicken has a SINGLE TemptGoal (chicken_food) — there is NO
# carrot_on_a_stick tempt (that is a PIG-only goal). The indices are CONSECUTIVE @0..@7 (8 goals); the
# pig's @2/@3-style gaps + the duplicate @4 Tempt do NOT apply to the chicken.
#
# CHICKEN EXTRAS ARE HOST aiStep WORK, NOT GOALS (34-00): the chicken slow-fall (deltaMovement.y *= 0.6
# while falling) + egg-lay (the eggTime timer → drop an egg + 2-nextFloat sound + nextInt(6000) reset)
# are net.minecraft.world.entity.animal.chicken.Chicken.aiStep server extras — a host hook
# (TickLoop.chickenAiStep, server/chicken_aistep.go) wired into the per-mob tick (tick_phases.go),
# entity.Chicken.ID-gated. They are NOT registerGoals entries, so they are NOT in the goals list below;
# this declaration only makes a spawned chicken EXIST as an entity.Chicken (so the host aiStep applies).
#
# Each goal callback receives (entity, world, nav) handles. The interpreter fires ONLY while a goal is
# RUNNING (an idle chicken makes zero starlark.Calls per tick). The draw ORDER below matches the
# bytecode + the proven Go goal runtime EXACTLY (the per-entity seeded RandomSource — 24-01 — makes a
# fixed seed reproduce the identical stream).

# --- constants (jar-confirmed) -----------------------------------------------------------------
FLOAT_JUMP_PROBABILITY = 0.8   # FloatGoal.tick: getRandom().nextFloat() < 0.8f (the swim-jump chance)
FLUID_JUMP_THRESHOLD = 0.4     # Entity.getFluidJumpThreshold: getEyeHeight()<0.4?0.0:0.4; the chicken
                               # eye >= 0.4 -> 0.4 (LOCKSTEP with the Go runtime's getFluidJumpThreshold).
                               # FloatGoal.canUse compares entity.fluid_height (WATER) > this. In the DRY
                               # world in_water is false so canUse short-circuits before this comparison.
...
STROLL_REDUCED_INTERVAL = 60   # reducedTickDelay(120) == Mth.positiveCeilDiv(120,2) == 60: canUse rolls
                               # nextInt(60), NOT nextInt(120) (the faithful gate; LOCKSTEP with the Go runtime)
...
PANIC_SPEED = 1.4         # Chicken.registerGoals @1 PanicGoal(mob, 1.4) (the want carries position only;
                               # the 1.4 multiplier is cited-deferred like stroll's speedModifier, LOCKSTEP
                               # with the Go runtime's panicSpeedModifier)
TEMPT_RANGE = 10.0        # Attributes.TEMPT_RANGE default (level/attribute/attributes.go) — the
                               # nearest-tempt-player scan radius the host applies. LOCKSTEP with the Go
                               # runtime's e.getAttributeValue(attribute.TemptRange).
TEMPT_SPEED = 1.0         # Chicken.registerGoals @3 TemptGoal(mob, 1.0, ..., false) speedModifier (the want
                               # carries position only; the 1.0 multiplier is cited-deferred like the others)
STOP_DISTANCE = 2.5       # TemptGoal DEFAULT_STOP_DISTANCE (the chicken uses the default ctor); tick stops
                               # the navigation within STOP_DISTANCE² (2.5² = 6.25), LOCKSTEP with the Go runtime
LOOK_DIST = 6.0           # LookAtPlayerGoal lookDistance (Chicken: 6.0f)
...
FOLLOW_RECALC_INTERVAL = 10    # FollowParentGoal.tick re-path cadence: adjustedTickDelay(10) (10 @ 20 TPS,
                               # NOT the halved 5). LOCKSTEP with the Go runtime's followRecalcInterval.

# ============================================================================================
# @0  FloatGoal(mob)   flags {JUMP}   requiresUpdateEveryTick=true
# ports net.minecraft.world.entity.ai.goal.FloatGoal — LOCKSTEP with the Go runtime (server/ai_goals_float.go)
# ============================================================================================
...
# the SAME predicates the Go runtime computes. In the DRY world in_water is false -> canUse false ->
# tick never runs -> zero new draws.
...
# with the Go runtime's mobRandom(e).nextFloat(). On a draw < 0.8 it arms the jump via nav.jump()
# (capNav -> jumpControl.doJump). FloatGoal@0 runs FIRST in the goal walk.
def float_tick(entity, world, nav):
    if entity.rand_float() < FLOAT_JUMP_PROBABILITY:   # DRAW 1: nextFloat()<0.8 (the swim-jump chance)
        nav.jump()

# ============================================================================================
# @1  PanicGoal(mob, 1.4)   flags {MOVE}
# ports net.minecraft.world.entity.ai.goal.PanicGoal — LOCKSTEP with the Go runtime (server/ai_goals_panic.go)
# ============================================================================================
...
# handle attrs entity.has_last_damage (the faithful not-null signal) + entity.damage_in_tag(
# "panic_causes") (the host-side DamageSource.is membership read; the tag id set stays on the Go side).
# The shouldPanic gate is the FIRST line so a dry/unhurt chicken draws ZERO RNG. When a panic DOES fire,
# findRandomPosition draws the UNCONDITIONAL 10-candidate DefaultRandomPos(5,4) loop: each candidate =
# generateRandomDirection in x, y, z ORDER (3 nextInt: nextInt(11)-5, nextInt(9)-4, nextInt(11)-5) = 30
# nextInt total, IDENTICAL to the Go runtime's stream. Emit the 10 RAW candidates PLUS landMode=0.0
# (DefaultRandomPos, no up-snap) as 31 FLAT positional floats via nav.path_to(x0,y0,z0,...,x9,y9,z9, 0.0).
def panic_can_use(entity, world, nav):
    ...
    # isOnFire/lookForWater false-stubs; a non-burning chicken never enters it.)
    flat = []
...
# stop (bytecode): isRunning = false. The Go runtime: clearWantTarget(). nav.stop() is that seam.
def panic_stop(entity, world, nav):
    nav.stop()

# canContinueToUse (bytecode): !navigation.isDone(). The Go runtime: e.ai.hasTarget. nav.has_path is that seam.
def panic_continue(entity, world, nav):
    return nav.has_path()

# ============================================================================================
# @2  BreedGoal(mob, 1.0)   flags {MOVE, LOOK}
# ports net.minecraft.world.entity.ai.goal.BreedGoal — LOCKSTEP with the Go runtime (server/ai_goals_breed.go breedGoal)
# ============================================================================================
# canUse (bytecode): if (!animal.isInLove()) return false; partner = getFreePartner(); return partner != null.
# The isInLove gate is the FIRST line — a lone un-fed adult chicken (inLove==0) returns false HERE, before
# the partner scan, so the breed path (and its host RNG draws via try_breed) NEVER fires unfed. is_in_love
# is the host read (Animal.isInLove); nearest_breeding_partner is the host scan (getFreePartner: nearest
# same-class in-love non-panicking within 8.0 — the same-class + canMate + isPanicking filter stays
# HOST-side). The .star sees only a partner position tuple or None. start() resets loveTime=0. NO RNG drawn
# in the .star (the breed draws are made host-side inside try_breed = TickLoop.breed — the lockstep rule).
...
# seam; move_to is the navigation.moveTo seam (the 1.0 speedModifier is the nav-tick multiplier,
# cited-deferred). The ++loveTime happens BEFORE the breed check (matching the Go runtime's tick order).
# At the threshold within 3 blocks the .star calls try_breed(8.0) — the ONE host breed op (= TickLoop.breed)
# that draws the variant nextBoolean() FIRST then the XP 1+nextInt(7) SECOND on the initiator's per-mob RNG.
...
# stop (bytecode): partner = null; loveTime = 0. The Go runtime: g.partner=nil; g.loveTime=0; clearWantTarget().
...
# The .star re-scans: nearest_breeding_partner returns a tuple iff a same-class in-love non-panicking partner
# is still in range, paired with loveTime < 60 (the courting bound) — behavior-identical to the Go runtime.
...
# ============================================================================================
# @3  TemptGoal(mob, 1.0, i -> i.is(ItemTags.CHICKEN_FOOD), false)   flags {MOVE, LOOK}
# ports net.minecraft.world.entity.ai.goal.TemptGoal — LOCKSTEP with the Go runtime (server/ai_goals_passive.go temptGoal)
# ============================================================================================
...
# player scan. The HOST owns the item-tag set + the scan: entity.nearest_player_holding_food("chicken_food",
# range) returns the matching player's (px,py,pz) tuple or None (the CHICKEN_FOOD tag membership stays
# Go-side — the 34-00 parameterized handle). The calmDown cooldown lives Go-side in the runtime's temptGoal;
# the .star re-scans each can_use, the observable equivalent. In the DRY world no player holds chicken_food
# near the chicken -> None -> can_use false -> zero new draws (TemptGoal draws zero RNG anyway).
def tempt_chickenfood_can_use(entity, world, nav):
    p = entity.nearest_player_holding_food("chicken_food", TEMPT_RANGE)   # host scan + predicate (ItemTags.CHICKEN_FOOD stays Go-side)
    if p == None:
        return False
...
# stopNavigation(); else navigateTowards(player) (moveTo(player, 1.0)). set_look_at is the LookControl
# .setLookAt seam; move_to is the navigateTowards seam (the 1.0 speedModifier cited-deferred).
def tempt_chickenfood_tick(entity, world, nav):
    ...
    if dx * dx + dy * dy + dz * dz < STOP_DISTANCE * STOP_DISTANCE:   # 2.5² = 6.25
        ...
        entity.move_to(px, py, pz)   # navigateTowards(player) @ TEMPT_SPEED 1.0

# stop (bytecode): player = null; stopNavigation(); calmDown = reducedTickDelay(100); isRunning = false.
def tempt_chickenfood_stop(entity, world, nav):
    nav.stop()   # stopNavigation()

# canContinueToUse (bytecode): if (canScare()) { ...flee-abort... } return canUse(). CHICKEN: canScare=false
# -> the flee block is DEAD -> canContinueToUse == canUse (re-scan).
def tempt_chickenfood_continue(entity, world, nav):
    return tempt_chickenfood_can_use(entity, world, nav)

# ============================================================================================
# @4  FollowParentGoal(mob, 1.1)   flags {} EMPTY
# ports net.minecraft.world.entity.ai.goal.FollowParentGoal — LOCKSTEP with the Go runtime (server/ai_goals_follow.go)
# ============================================================================================
...
# is_baby is the host read (AgeableMob.isBaby == age<0) — the FIRST-line gate: a lone ADULT chicken
# (is_baby false) returns false HERE, so the follow path is dormant on an adult. nearest_adult_parent is the
# host scan (the nearest same-class ADULT in inflate(8,4,8) NOT within 3 blocks). NO RNG anywhere.
...
# navigation.moveTo(parent, speed). PURE INT — NO RNG. move_to is the navigation.moveTo(parent, 1.1) seam.
...
# stop (bytecode): parent = null. The Go runtime: g.parent=nil; clearWantTarget().
...
# gate (DRAW 1: nextInt(60)), THEN — for a not-in-water chicken — the probability nextFloat() gate (DRAW 2:
...
# candidate = generateRandomDirection in x, y, z ORDER (3 nextInt). 31 RNG draws total (gate + probability +
# 30 offsets). Emit the 10 RAW candidates PLUS the wantLandMode flag as 31 FLAT positional floats via
# nav.path_to(x0,y0,z0,...,x9,y9,z9, landMode). DRAW ORDER must match the Go runtime (gate, probability,
# then xt/yt/zt per candidate).
def stroll_can_use(entity, world, nav):
    ...
    # RARE, no up-snap). Carry the SAME bool the Go goal computes; pass it as the 31st float (1.0/0.0).
...
# start: the candidates are committed in can_use (above), so start is a no-op (the goal drops the start kwarg).

# canContinueToUse (bytecode): !navigation.isDone(). The Go runtime: e.ai.hasTarget. nav.has_path is that seam.
def stroll_continue(entity, world, nav):
    return nav.has_path()

# stop (bytecode): navigation.stop(). The Go runtime: clearWantTarget(). nav.stop() is that seam.
def stroll_stop(entity, world, nav):
    nav.stop()

# RandomStrollGoal.tick() is EMPTY in the jar (its behavior is start()=moveTo + continue=!isDone); no tick.
...
# lookDistance -> lookAt; return lookAt != null. The Go runtime reads the nearest player from loop.players
# via nearestPlayerWithin(e, dist) using e.x/e.y/e.z (NOT eye_y) — so this plugin uses entity.y for the scan.
...
# start (bytecode): lookTime = adjustedTickDelay(40 + getRandom().nextInt(40)). The Go runtime:
...
# lookTime > 0. The Go runtime: hasLook && lookTime>0 && d2 <= lookDistance^2.
...
# lookTime = lookTime - 1. set_look_at is the LookControl.setLookAt analogue. Order: set look FIRST,
# THEN decrement (matches the bytecode + the Go runtime).
...
# stop (bytecode): lookAt = null. The Go runtime: hasLook=false. Clearing the captured target.
...
# Go runtime's d = 2*pi*r.nextDouble() EXACTLY, so the RNG stream stays identical.
...
# getZ()+relZ). The Go runtime: lookTime--; e.headYaw=e.yaw=yawTowardDeg(relX, relZ). Order: decrement
# FIRST, then aim (matches the bytecode + the Go runtime).
...
# --- the declaration ---------------------------------------------------------------------------
# base_type "chicken" -> renders as entity.Chicken.ID (custom = BEHAVIOR, not a new wire type); the
# 0.4x0.7 chicken hitbox auto-applies via NewEntity(decl.baseType). Real chicken attributes
# (Chicken.createAttributes: max_health 4.0, movement_speed 0.25 — 34-JARNOTES.md:72,323). The 8 goals at
# the EXACT jar priorities + flags (Chicken.registerGoals, 34-JARNOTES.md:38-48). The slow-fall + egg-lay
# are HOST aiStep extras (TickLoop.chickenAiStep, 34-00) — NOT goals, so they are NOT in this list.
declare_mob(
    name = "vanilla_chicken",
    base_type = "chicken",
    attributes = {
        "max_health": 4.0,       # Chicken.createAttributes: MAX_HEALTH 4.0
        "movement_speed": 0.25,  # Chicken.createAttributes: MOVEMENT_SPEED 0.25
    },
    goals = [
        ...
        # Cite Chicken.registerGoals @0 FloatGoal.
        goal(
            priority = 0,
            flags = ["JUMP"],
        ...
        # @1 PanicGoal(mob, 1.4) [MOVE] — can_use commits the candidates (via path_to) so NO start kwarg.
        # Cite Chicken.registerGoals @1 PanicGoal(1.4).
        goal(
            priority = 1,
            flags = ["MOVE"],
        ...
        # @2 BreedGoal(mob, 1.0) [MOVE, LOOK] — can_use gates on is_in_love (dormant unfed); tick courts the
        # partner and at the threshold routes the breed through the ONE host try_breed (= TickLoop.breed).
        # Cite Chicken.registerGoals @2 BreedGoal(1.0).
        goal(
            priority = 2,
            flags = ["MOVE", "LOOK"],
        ...
        # @3 TemptGoal(mob, 1.0, CHICKEN_FOOD, false) [MOVE, LOOK] — the SINGLE chicken tempt (chicken_food
        # tag; NO carrot_on_a_stick goal). Cite Chicken.registerGoals @3 TemptGoal(1.0, is(CHICKEN_FOOD)).
        goal(
            priority = 3,
            flags = ["MOVE", "LOOK"],
            can_use = tempt_chickenfood_can_use,
            tick = tempt_chickenfood_tick,
            stop = tempt_chickenfood_stop,
            can_continue = tempt_chickenfood_continue,
        ),
        # @4 FollowParentGoal(mob, 1.1) [] EMPTY flags — a baby trails the nearest adult; re-paths every 10
        # ticks, draws NO RNG. can_use gates on is_baby (dormant on an adult). Cite Chicken.registerGoals
        ...
        # @5 WaterAvoidingRandomStrollGoal(1.0) [MOVE] — can_use commits the candidates (via path_to), so NO
        # start kwarg. Cite Chicken.registerGoals @5 WaterAvoidingRandomStrollGoal(1.0).
        goal(
            priority = 5,
            flags = ["MOVE"],
        ...
        # @6 LookAtPlayerGoal(Player, 6.0) [LOOK]. Cite Chicken.registerGoals @6 LookAtPlayerGoal.
        goal(
            priority = 6,
            flags = ["LOOK"],
        ...
        # @7 RandomLookAroundGoal [MOVE, LOOK] — requiresUpdateEveryTick=true (jar-confirmed). Cite
        # Chicken.registerGoals @7 RandomLookAroundGoal.
        goal(
            priority = 7,
            flags = ["MOVE", "LOOK"],
