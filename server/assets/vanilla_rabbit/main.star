# vanilla_rabbit — a 1:1 vanilla-Rabbit dogfood (MOB-PASS-05), the Rabbit built as a Starlark plugin. A
# LITERAL method-for-method port of the unobfuscated 26.2 jar (temp/cache/26.2-inner.jar, javap -c -p). It
# declares the PORTABLE slice of net.minecraft.world.entity.animal.rabbit.Rabbit.registerGoals, reusing the
# proven mob-agnostic goal callbacks (float/panic/breed/tempt/stroll/look — COPIED VERBATIM from vanilla_cow;
# only the constants + the tempt food tag + the declaration differ).
#
...
#   @5  RaidGardenGoal                                  <-- DEFERRED (no carrot-crop / garden subsystem)
#   @6  WaterAvoidingRandomStrollGoal(0.6)              <-- .star (shared stroll)
...
LOOK_DIST = 10.0          # LookAtPlayerGoal lookDistance (Rabbit: 10.0f)
...
# player scan. The HOST owns the item-tag set + the scan: entity.nearest_player_holding_food("rabbit_food",
...
def tempt_food_can_use(entity, world, nav):
    p = entity.nearest_player_holding_food("rabbit_food", TEMPT_RANGE)   # host scan + COW_FOOD predicate (Go-side)
    if p == None:
        return False
...
# --- the declaration ---------------------------------------------------------------------------
# base_type "rabbit" -> renders as entity.Rabbit.ID. Real Rabbit attributes (Animal.createAnimalAttributes
# + Rabbit overrides) seeded with the jar values (Rabbit.createAttributes: MAX_HEALTH 3.0, MOVEMENT_SPEED
# 0.3, ATTACK_DAMAGE 3.0). The PORTABLE goal slice at the jar registerGoals priorities (float@1, panic@1,
# breed@2, tempt@3, stroll@6, look@11); the avoid/raid-garden/powder-snow goals are cite-deferred (header).
# Cite net.minecraft.world.entity.animal.rabbit.Rabbit.registerGoals + Rabbit.createAttributes.
declare_mob(
    name = "vanilla_rabbit",
    base_type = "rabbit",
    attributes = {
        "max_health": 3.0,       # Rabbit.createAttributes: MAX_HEALTH 3.0
        "movement_speed": 0.3,   # Rabbit.createAttributes: MOVEMENT_SPEED 0.3
        "attack_damage": 3.0,    # Rabbit.createAttributes: ATTACK_DAMAGE 3.0 (the killer-bunny bite; benign otherwise)
    },
    goals = [
        # @1 FloatGoal [JUMP] — requiresUpdateEveryTick=true. Cite Rabbit.registerGoals @1 FloatGoal.
        goal(
            priority = 1,
            flags = ["JUMP"],
        ...
        # @2 BreedGoal(mob, 0.8) [MOVE, LOOK]. Cite Rabbit.registerGoals @2 BreedGoal(0.8).
        goal(
            priority = 2,
            flags = ["MOVE", "LOOK"],
        ...
        # @3 TemptGoal(mob, 1.0, RABBIT_FOOD, false) [MOVE, LOOK]. Cite Rabbit.registerGoals @3 TemptGoal.
        goal(
            priority = 3,
            flags = ["MOVE", "LOOK"],
        ...
        # @6 WaterAvoidingRandomStrollGoal(mob, 0.6) [MOVE]. Cite Rabbit.registerGoals @6.
        goal(
            priority = 6,
            flags = ["MOVE"],
        ...
        # @11 LookAtPlayerGoal(Player, 10.0) [LOOK]. Cite Rabbit.registerGoals @11 LookAtPlayerGoal(Player, 10.0).
        goal(
            priority = 11,
            flags = ["LOOK"],
