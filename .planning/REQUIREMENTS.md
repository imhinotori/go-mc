...
- [ ] **FEAT-03**: The core overworld feature types are ported: `OreFeature` (decoration ores, distinct from v1's noise OreVeinifier), `RandomPatchFeature` + `SimpleBlockFeature` (grass/flowers/ground cover), the `RandomSelector`/`SimpleRandomSelector`/`RandomBooleanSelector` composites, `BlockPile`/`FallenTree`/`VegetationPatch` — so a biome's full vanilla ground-cover + ore set generates. _(12-01 landed the shared prerequisites: BlockStateProvider hierarchy, random_offset/block_predicate_filter + the TrapezoidalInt IntProvider, NextBoolean/NextGaussian, the featureBody registry. The feature BODIES land in 12-02/12-03.)_
