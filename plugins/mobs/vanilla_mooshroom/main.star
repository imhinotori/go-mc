# vanilla_mooshroom — a 1:1 vanilla-MushroomCow dogfood (MOB-PASS-04), the Mooshroom built as a Starlark
# plugin. net.minecraft.world.entity.animal.cow.MushroomCow extends AbstractCow and OVERRIDES neither
# registerGoals NOR createAttributes (javap-verified: MushroomCow has no registerGoals/createAttributes
# method — it inherits AbstractCow's), so a Mooshroom's AI + attributes are IDENTICAL to a Cow's. This
# plugin is therefore the vanilla_cow declaration verbatim with base_type "mooshroom" (renders as
# entity.Mooshroom.ID) — the SAME 8-goal AbstractCow.registerGoals set, the SAME max_health 10 /
# movement_speed 0.2 attributes. The mooshroom-specific bits (shear-to-cow, mushroom-stew mobInteract,
# variant/effect) are HOST-side interacts (not goals) and are cite-deferred until that interact lands.
# Cite net.minecraft.world.entity.animal.cow.MushroomCow (extends AbstractCow, no goal/attr override).
#
