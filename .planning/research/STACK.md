...
**Domain:** High-performance Minecraft Java Edition server (v26.2 / protocol 776) in Go
**Researched:** 2026-06-23
**Confidence:** HIGH on core/protocol foundation and concurrency stack; MEDIUM on noise/world-gen library choice (vanilla parity is bespoke); HIGH on the fork decision.

> Builds on `.planning/PROJECT.md` Context. This document does NOT re-derive the #294–296 pipeline facts already established there — it verifies them against live sources, decides fork-vs-vendor, and pins the supporting Go library stack with versions and rationale.

---

## TL;DR Decisions

1. **Fork `Tnze/go-mc` — do not vendor, do not depend on upstream `@master`.** Upstream master is pinned at MC 1.21 (last touched Dec 2024) and the codegen pipeline (#294–296) lives in unmerged *draft* PRs targeting 1.21.11 / proto 774. You are building for 26.2 / proto 776, which is ahead of both. You must own the codegen retarget. Fork is mandatory.
2. **Concurrency stack = `puzpuzpuz/xsync/v4` + `panjf2000/ants/v2` + `sourcegraph/conc`.** xsync is the Go answer to Leaf's FastUtil + lock-free queues (CLHT map, Vyukov MPMCQueue). ants is the bounded goroutine pool for async pathfinding/tracker/spawning. conc is structured-concurrency glue for the tick loop. Avoid bespoke lock-free code unless profiling demands it.
3. **World-gen noise: write a Mojang-faithful port, do not adopt an off-the-shelf OpenSimplex lib for vanilla parity.** Minecraft uses improved-Perlin / OctaveSimplex (custom), not OpenSimplex. Use `ojrac/opensimplex-go`-style libs only for the *non-vanilla deterministic stub generator* (the MVP requirement), never for parity work.
4. **Java 25 is a build-time-only tool.** It runs the jar extractors inside a container during codegen. It is NOT a runtime dependency and must never enter the server's `go.mod` or Docker runtime image.
5. **Codegen tooling = the go-mc `tools/` second-module pattern**, invoked `cd tools && go run . --version 26.2`, with Java extractors in an `eclipse-temurin:25-jdk` (or Zulu 25) container.
...
**Concretely:** fork to e.g. `github.com/<you>/go-mc`, branch off upstream `master`, cherry-pick the #294–296 commits (or rebase them — they're drafts so expect conflicts), swap the extractor container to JDK 25, run `cd tools && go run . --version 26.2`, then hand-patch `net`/packet framework for the 776 differences enumerated in PROJECT.md.
...
**If you are at the MVP "first playable connection" milestone:**
...
**If you are at the Leaf-parity / async optimization milestone:**
...
**If/when retargeting to a future drop (26.3, proto 777+):**
...
---
*Stack research for: high-performance Minecraft Java 26.2 server in Go*
*Researched: 2026-06-23*
