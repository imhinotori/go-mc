...
// DefaultStateID maps a block id to its defaultBlockState() state id (the
// registerDefaultState value). Use this — not ToStateID[FromID[id]] — for placement,
// because the Go zero-value struct is not a valid state for blocks with non-zero
// default properties (chest facing, etc).
var DefaultStateID = map[string]StateID{
	"minecraft:air":                                 0,
	"minecraft:stone":                               1,
	"minecraft:granite":                             2,
