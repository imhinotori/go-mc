# vanilla_bogged - a 1:1 vanilla-Bogged dogfood (MOB-VARIANT), the Bogged built as a Starlark plugin.
# net.minecraft.world.entity.monster.skeleton.Bogged extends AbstractSkeleton and OVERRIDES createAttributes
# (javap-verified: AbstractSkeleton.createAttributes() then MAX_HEALTH 16.0) -- otherwise its HOSTILE AI is
# a Skeleton's. base_type "bogged" (renders as entity.Bogged.ID). Cite Bogged (extends AbstractSkeleton,
# implements Shearable).
#
# THE SIGNATURE (Go-native, zombie_variants.go): Bogged.getArrow adds a POISON 100 (5s) MobEffectInstance
# to the fired Arrow (amplifier 0). Bogged is Shearable -- shearing drops from the shearing/bogged loot
# table (spawnShearedMushrooms). The ranged path is the shared kind="ranged_bow_attack"; the tipped effect
# rides the arrow's carried effects (boggedArrowEffects). Cite Bogged.getArrow -> arrow.addEffect(new
# MobEffectInstance(POISON, 100)) + Bogged.createAttributes (MAX_HEALTH 16) + Bogged.shear.
#
# DEFERRED (cite-recorded): the shear loot table (minecraft:shearing/bogged) has no loot-table subsystem in
# v1 -- the shear drop is a cited RED_MUSHROOM stub in zombie_variants.go (spawnShearedMushrooms). Cite
# Bogged.spawnShearedMushrooms -> dropFromShearingLootTable(BOGGED_SHEAR).
...
declare_mob(
    ...
    base_type = "bogged",
    attributes = {
        "max_health": 16.0,      # Bogged.createAttributes: MAX_HEALTH 16.0 (the override)
