...
**Plans**: 3 plans
  - [ ] 13-01-PLAN.md — Port TreeFeature.place + StraightTrunkPlacer + BlobFoliagePlacer + TreeConfiguration parse + the empty-decorator baseline so oak/birch forests render real trees, wired to the cross-chunk Neighborhood + the Phase-12 selector recursion (FEAT-04)
  - [ ] 13-02-PLAN.md — Port the remaining overworld trunk/foliage placers (Forking/Fancy/DarkOak/MegaJungle/Spruce/Pine/Acacia/Bush/...) + the tree decorators the overworld uses (AlterGround/Beehive/Cocoa/vines) + the all-overworld-tree-configs-decode guard so every biome grows its correct tree set (FEAT-04)
  - [ ] 13-03-PLAN.md — Port the MonsterRoomFeature dungeon (cobble room + spawner + chests as blocks, loot deferred) + the automated full-features acceptance (trees+dungeon+selector+5x5 determinism+Docker -race) + THE autonomous:false real-client VISUAL GATE (FEAT-05, FEAT-06)
...
| 13. Trees, Dungeon & Features Visual Gate | 0/3 | Planned | - |
