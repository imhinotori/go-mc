...
- [x] 07-02-PLAN.md — AI-02: port the request→snapshot→result A* (PathNavigationRegion snapshot + PathFinder + WalkNodeEvaluator + GroundPathNavigation), the pure computePath seam, wired into serverAiStep (wave 2)
- [x] 07-03-PLAN.md — AI-03: port the faithful-minimal NaturalSpawner + MobCategory cap accounting, fill tickAI() (drive serverAiStep + spawner), retire the sinusoidal debug pig (wave 3)
...
- [x] 08-02-PLAN.md — OPT-01 async pathfinding (Wave 1): requestPath submits computePath to the pathPool and continues; pathReady rejoins with despawn/retarget validation; navigation tolerates a 1+-tick-late path (OPT-01/06)
- [x] 08-03-PLAN.md — OPT-05 linear region format (Wave 1): save/region/linear.go (whole-region zstd, pure-Go klauspost/compress) alongside .mca; format-aware opt-in loadOrGenerate; round-trip + bomb/corrupt guards (OPT-05/06)
- [x] 08-04-PLAN.md — OPT-02 async tracker (Wave 2): asyncTracker behind the unchanged tracker.Tick() seam (one swap-point line); diff math off-tick over a snapshot; owner-side packet emission; golden equivalence vs entityTracker (OPT-02/06)
- [x] 08-05-PLAN.md — OPT-03 async mob spawning (Wave 3): naturalSpawn scan off-tick over a solidity snapshot; spawnCandidatesReady re-checks the cap on the owner before entityStore.add; every anti-flood/anti-piling guard survives (OPT-03/06)
- [x] 08-06-PLAN.md — OPT-04 collections contention audit (Wave 4): snapshot-and-stay-plain per collection (justified); ants pools confirmed; one justified xsync/v4 usage; the no-live-capture discipline gate (OPT-04/06)
- [x] 08-07-PLAN.md — OPT-06 final gate (Wave 5): combined all-subsystems concurrent stress + behavior-regression (paths arrive / tracker sends / mobs spawn / .linear round-trips); the Docker -race -count=10 phase gate green; automatable, no new wire surface (OPT-06)
...
**In scope now (planned)**: PARITY-01 ONLY — the user scoped Phase 9 to vanilla-parity worldgen. ONLINE-01, ONLINE-02, and REGION-01 are DEFERRED (tracked, not planned in this round).
**Success Criteria** (what must be TRUE):
  1. Mojang/Microsoft account authentication and protocol encryption work for online-mode connections (ONLINE-01, ONLINE-02) — DEFERRED
  2. Mojang density-function world generation produces terrain with FULL vanilla parity (PARITY-01) — a real vanilla 26.2 client explores recognizable, BIOME-VARIED overworld terrain (hills/valleys/plains/mountains + desert/ocean, water at sea level y=63) WITH working cave systems (noise caves + carved tunnels), RAVINES, underground water/lava AQUIFERS, and ORE VEINS, generated by the real ported density-function graph + aquifer + ore-veinifier + carvers + multi-noise biomes behind the unchanged Generator seam (FULL-parity scope; mineshafts-as-structures + trees/vegetation features are the documented deferrals)
  3. Folia-style per-region tick threading runs on top of the ownership-isolated core without a rewrite (REGION-01) — DEFERRED
**Research**: `.planning/phases/09-stretch-online-worldgen-regions/09-RESEARCH.md` (PARITY-01 — the levelgen port order, the data-driven graph, the Xoroshiro seeding chain, the scope tiers, the visual+determinism validation). Scope upgraded from T0-surface to FULL parity (caves/ravines/aquifers/ore-veins/multi-noise-biomes/full-surface-rules): the data-driven graph means the cave functions are already in the embedded `final_density`, so CAVES come "free" once the full node-type set evaluates the whole overworld graph — the incremental cost is the full node set + Aquifer + OreVeinifier + carvers + SurfaceRules + Climate (all ported from the jar). Mineshafts-as-structures + trees/vegetation features are deferred (separate large subsystems).
**Plans (PARITY-01):** 9 plans in 7 waves
- [ ] 09-01-PLAN.md — Wave 1: extract+embed the FULL worldgen DATA (noise_settings + ALL density_function incl. the cave functions + noise + configured_carver + carver-replaceables tag + the baked biome params) — the DATA half (caves are in the graph)
- [ ] 09-02-PLAN.md — Wave 1: Tier A Xoroshiro seeding chain + Tier B noise primitives — the determinism hinge, ported + golden-tested first
