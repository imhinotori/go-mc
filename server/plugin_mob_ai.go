...
func buildNativeGoal(kind string, gd goalDecl, decl *mobDecl) Goal {
	...
	case "cat_avoid_player":
		// MOB-NEUT-03: Cat$CatAvoidEntityGoal<Player>(this, 16.0f, 0.8d, 1.33d) — {MOVE}. A WILD (untamed)
		// cat flees players; canUse/canContinueToUse gate on !isTame() so a tamed cat never fires it
		// (reproducing Cat.reassessTameGoals' remove-on-tame). Added at @4 while untamed. Cite
		// Cat.reassessTameGoals + Cat$CatAvoidEntityGoal (ai_goals_cat.go).
		return newCatAvoidPlayerGoal()
	case "sit":
