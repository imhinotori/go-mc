...
type TickLoop struct {
	...
	// dirtyRelight is the per-tick BATCHED-RELIGHT dirty queue (the fluid-spread stall fix, CR-01/WR-01
	// closure). relightChanged no longer recomputes light + broadcasts INLINE on every light-affecting
	// SetBlock (fluid simulation writes thousands of blocks/tick, and each inline RelightEdit
	// recomputed 9 full chunk columns via the light engine -- a 100-140ms stall per fluid cell).
	// Instead it ENQUEUES the EDITED column here via relightQueue.add under a mutex; many edits in
	// one column collapse to one entry (the value is any representative block pos in the column,
	// which is all RelightColumns needs). flushRelight (region_coordinator.go tickOnce, post-BE /
	// pre-movement) drains this ONCE per tick via relightQueue.drain (atomic swap, lock released
	// before the expensive RelightColumns + packet send): it builds the UNION of each dirty edited
	// column + its 8 neighbors in DETERMINISTIC (dim, cx, cz) order, recomputes each affected column
	// exactly once over the FINAL block states, broadcasts one ClientboundLightUpdate per changed
	// column, and merges any budget-deferred columns back via relightQueue.mergeCarry. Light is a
	// pure function of the final states, so end-of-tick recompute is byte-identical to per-edit
	// recompute (the last recompute wins) -- and matches vanilla, which coalesces checkBlock nodes
	// and runs runLightUpdates ONCE per tick, then broadcasts each column's ClientboundLightUpdate
	// once. Written from ANY goroutine (relightChanged is invoked from SetBlock on the region's fan-
	// out goroutine under the N=2 model); drained on the coordinator (flushRelight). Always non-nil
	// in NewTickLoop. CITE: LevelChunk.setBlockState checkBlock ->
	// ThreadedLevelLightEngine.runLightUpdates -> ChunkMap ClientboundLightUpdate (once per tick).
	dirtyRelight *relightQueue
...
func NewTickLoop(clock Clock) *TickLoop {
	...
	// dirtyRelight is the per-tick batched-relight dirty queue (race-safe relightQueue). Constructed
	// here so it is non-nil before the first tick even if installRelightHook has not yet run.
	// Drained by flushRelight in tickOnce.
	t.dirtyRelight = newRelightQueue()
