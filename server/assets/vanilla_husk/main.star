# vanilla_husk — a 1:1 vanilla-Husk dogfood (MOB-HOST-04), the Husk built as a Starlark plugin.
# net.minecraft.world.entity.monster.zombie.Husk extends Zombie and OVERRIDES neither registerGoals NOR
# createAttributes (javap-verified: Husk's only ctor calls super(...); it adds no goal/attribute method),
# so a Husk's HOSTILE AI + attributes are IDENTICAL to a Zombie's. This plugin is therefore the
# vanilla_zombie declaration verbatim with base_type "husk" (renders as entity.Husk.ID) — the SAME goal
# set (Go-native melee/hurt_by/nearest_attackable + the .star passive stroll/look/around) and the SAME
# Zombie.createAttributes values (FOLLOW_RANGE 35, MOVEMENT_SPEED 0.23, ATTACK_DAMAGE 3, ARMOR 2). The
# Husk-specific bits (no sun-burn, the isSunSensitive override, hunger effect on hit) are cite-deferred:
# isSunSensitive is a Go-side gate (fire.go) that stays false for the husk until wired. Cite
# net.minecraft.world.entity.monster.zombie.Husk (extends Zombie, no goal/attr override).
#
